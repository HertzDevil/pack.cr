...
describe Pack do
  describe ".pack" do
    ...
    describe "w" do
      it "packs BER-compressed integers" do
        it_packs "w", 0, Bytes[0x00]
        it_packs "w", 7, Bytes[0x07]
        it_packs "w", 100, Bytes[0x64]
        it_packs "w", 0x8A_u64, Bytes[0x81, 0x0A]
        it_packs "w", 0x2000_u64, Bytes[0xC0, 0x00]
        ...
        it_packs "w2", iterator_of(130, 131_u8, 132_u16), Bytes[0x81, 0x02, 0x81, 0x03]

        expect_raises(IndexError) { Pack.pack "w2", {1} }
        expect_raises(IndexError) { Pack.pack "w3", [1, 2] }
